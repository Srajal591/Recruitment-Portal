...
const SupportKanban = () => {
  ...
  const countByStatusName = (name) =>
    statusStats.find((s) => s._id === name)?.count || 0;
  const stats = {
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 text-sm">
              Drag-free kanban view of all support tickets.
            </p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            {
              label: "Open",
              value: stats.open || grouped["Open"]?.length || 0,
              color: "text-red-600",
            },
            {
              label: "In Progress",
              value: stats.inProgress || grouped["In Progress"]?.length || 0,
              color: "text-yellow-600",
            },
            {
              label: "Resolved",
              value: stats.resolved || grouped["Resolved"]?.length || 0,
              color: "text-green-600",
            },
            {
              label: "Closed",
              value: stats.closed || grouped["Closed"]?.length || 0,
              color: "text-gray-600",
            },
          ].map((s) => (
                    ...
                    {colTickets.map((ticket) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2 mb-2">
                          ...
                          <Badge
                            className={`${PRIORITY_COLORS[ticket.priority] || "bg-gray-100 text-gray-800"} text-xs flex-shrink-0`}
                          >
                            {ticket.priority}
                          </Badge>
                        </div>

                        <p className="text-xs text-orange-600 font-mono mb-2">
                          {ticket.ticketId}
                        </p>
