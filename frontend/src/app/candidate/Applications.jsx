...
const Applications = () => {
  ...
  const total = data?.pagination?.totalItems || applications.length
    ...
    <CandidateLayout title="My Applications">
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          ...
          <Button onClick={() => navigate('/jobs')} className="bg-orange-600 hover:bg-orange-700 text-white">
            Browse Jobs
          </Button>
        </div>
          ...
          <CardContent className="p-0">
            ...
            {!isLoading && applications.length === 0 && (
              <div className="p-8 text-center">
                ...
                <p className="text-gray-400 text-sm mt-1">Browse available jobs and start applying</p>
                <Button onClick={() => navigate('/jobs')} className="mt-4 bg-orange-600 hover:bg-orange-700 text-white">
                  Browse Jobs
                </Button>
              </div>
            )}
            {applications.map((app) => {
              ...
              return (
                <div key={app._id} className="flex items-center justify-between p-4 border-b border-gray-100 last:border-0 hover:bg-orange-50">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-orange-100 rounded-lg flex items-center justify-center flex-shrink-0">
                      <FileText className="w-5 h-5 text-orange-600" />
                    </div>
                    <div>
                      <p className="font-medium text-gray-800">{app.jobId?.title || 'Job Application'}</p>
                      <p className="text-sm text-gray-500">{app.jobId?.department || '—'}</p>
                      <p className="text-xs text-orange-600 mt-0.5">{app.applicationId}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
