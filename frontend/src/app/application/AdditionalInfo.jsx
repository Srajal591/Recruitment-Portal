...
const AdditionalInfo = () => {
  ...
  const { mutate: saveStep, isPending } = useMutation({
    mutationFn: (data) => candidateService.saveAdditionalInfo(applicationId, data),
    onSuccess: () => {
  ...
  const handleNext = () => {
    ...
    if (formData.isGovtEmployee) {
      ...
      if (formData.yearsOfService) payload.yearsOfService = Number(formData.yearsOfService)
    }
    if (formData.isPwD) {
      if (formData.disabilityType) payload.disabilityType = formData.disabilityType
      if (formData.disabilityPercentage) payload.disabilityPercentage = Number(formData.disabilityPercentage)
    }
    if (formData.drivingLicense) payload.drivingLicense = formData.drivingLicense
    if (formData.computerCertificate) payload.computerCertificate = formData.computerCertificate
    if (formData.subjectCombination) payload.subjectCombination = formData.subjectCombination
    saveStep(payload)
  }
    ...
    <ApplicationLayout currentStep={3} title="Additional Details">
      <div className="space-y-6">
        <Card className="shadow-sm">
          ...
          <CardContent className="space-y-8">
            ...
            <div className="border-l-4 border-orange-400 pl-6">
              <h3 className="text-lg font-medium text-gray-800 mb-4">Government Employment Status</h3>
                ...
                {formData.isGovtEmployee && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <div>
                      ...
                      <input type="text" className={inputCls} placeholder="e.g. Department of Education"
                        value={formData.departmentName} onChange={(e) => set('departmentName', e.target.value)} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Years of Service</label>
                      <input type="number" min="0" className={inputCls} placeholder="4"
                        value={formData.yearsOfService} onChange={(e) => set('yearsOfService', e.target.value)} />
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="border-l-4 border-yellow-400 pl-6">
              <h3 className="text-lg font-medium text-gray-800 mb-4">Special Category Details</h3>
              <div className="space-y-6">
                <div>
                  <p className="text-sm font-medium text-gray-700 mb-3">Ex-serviceman?</p>
                  <YesNo field="isExServiceman" />
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-700 mb-3">Person with Disability (PwD)?</p>
                  <YesNo field="isPwD" />
                  {formData.isPwD && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">Disability Type</label>
                        <select className={inputCls} value={formData.disabilityType} onChange={(e) => set('disabilityType', e.target.value)}>
                      ...
                      <div>
                        ...
                        <input type="number" min="0" max="100" className={inputCls} placeholder="45"
                          value={formData.disabilityPercentage} onChange={(e) => set('disabilityPercentage', e.target.value)} />
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <select className={inputCls} value={formData.drivingLicense} onChange={(e) => set('drivingLicense', e.target.value)}>
                  <option value="">Select</option>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Computer Certificate</label>
                <select className={inputCls} value={formData.computerCertificate} onChange={(e) => set('computerCertificate', e.target.value)}>
                  <option value="">Select</option>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Subject Combination</label>
                <select className={inputCls} value={formData.subjectCombination} onChange={(e) => set('subjectCombination', e.target.value)}>
                  <option value="">Select</option>
        ...
        <div className="flex justify-between">
          <Button variant="outline" onClick={() => navigate('/application/education')}>← Back</Button>
